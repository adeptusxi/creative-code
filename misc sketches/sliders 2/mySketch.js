...
let solidChance = 0.8; // chance for big slider to be solid (not hollow)
...
function draw() {
	bg60 ? background(adjustAlpha(col60, 30)) : background(adjustAlpha(col30, 30));
  let prevSlider = null;
  for (let s of sliders) {
    s.update();
    s.displayConnectors(prevSlider); 
    prevSlider = s;
  }
  for (let s of sliders) {
    s.display(); 
...
function keyPressed() {
	if (keyCode != 32) return; // not space
	looping ? noLoop() : loop(); 
	looping = !looping; 
}
...
function generateColors() {
	...
	col30 = chroma((h + random([-30, 30])) % 360, 0.15, 0.8, "hsv").hex(); // pastel secondary 
