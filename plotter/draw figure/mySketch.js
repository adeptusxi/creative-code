...
function getCapsuleVerts(cx, cy, w, h, angle, widen=true, wiggle=2) {
	let sideSteps = h/15;
	let verts = [];
	let r = w / 2;
...
function getCircleVerts(cx, cy, r, wiggle=2, numPoints=20) {
  let verts = [];
...
function drawPersonUnioned(x, y, size) {
  ...
  //left arm 
	capsules.push(getCapsuleVerts(-torsoW/2 - limbW/2, torsoTop + limbW*0.8, limbW, armH, radians(random(30, 50))));
  // right arm 
	capsules.push(getCapsuleVerts(torsoW/2 + limbW/2, torsoTop + limbW*0.8, limbW, armH, radians(-random(30, 50)))); 
  // left leg 
	capsules.push(getCapsuleVerts(-torsoW/4 - limbW*0.6, torsoH/2 + legH/2, limbW, legH, radians(random(15, 35))));
  // right leg 
	capsules.push(getCapsuleVerts(torsoW/4 + limbW*0.6, torsoH/2 + legH/2, limbW, legH, radians(-random(15, 35)))); 
  ...
  translate(x, y);
  beginShape();
  for (let v of clipperPathToP5(solution_paths[0], scale)) {
    vertex(v.x, v.y);
  }
  endShape(CLOSE);
  pop();
